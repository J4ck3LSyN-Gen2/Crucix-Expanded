...
### Cyber Intelligence Keys (optional)

| Key | Used For | Notes |
|-----|----------|-------|
| `NIST_API_KEY` | Higher-rate NIST NVD CVE queries | Optional. NIST works without a key but at lower public rate limits. |
| `GREYNOISE_API_KEY` | GreyNoise watchlist enrichment | Optional. Improves reliability and limits for Community lookups. |
| `GREYNOISE_WATCHLIST` | Comma-separated IPs to monitor | Required only if you want GreyNoise watchlist checks (example: `8.8.8.8,1.1.1.1`). |

> No key is needed for CISA KEV, FIRST EPSS, or the expanded DDoS/ransomware/cyberattack RSS OSINT feeds.
...
### Tier 6: Cyber Intelligence (4)

| Source | What It Tracks | Auth |
|--------|---------------|------|
| **NIST NVD** | High/critical CVEs and exploit-relevant vulnerability metadata | Optional API key |
| **CISA KEV** | Known exploited vulnerabilities + CISA cyber advisories | None |
| **FIRST EPSS** | Exploit probability ranking for CVEs | None |
| **GreyNoise Community** | Watchlist checks for noisy/malicious internet scanner activity | Optional API key |
